...
export function RecurringPageClient({ isProPlan }: Props) {
  const [items, setItems] = useState<RecurringInvoice[]>([])
  const [loading, setLoading] = useState(true)
  const [showForm, setShowForm] = useState(false)

  useEffect(() => {
    if (!isProPlan) { setLoading(false); return }
    fetch('/api/recurring')
      .then(r => r.json())
    ...
    return (
      <div className="space-y-6 max-w-4xl">
        ...
        <ProUpsell
          ...
          description="Nastav šablonu jednou a Fakturo ji sám podle plánu vystaví a odešle klientovi — součást Pro plánu."
        />
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        ...
        {!showForm && (
          <button
            ...
            className="inline-flex items-center gap-2 bg-brand text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-brand-dark transition"
          >
      ...
      {showForm && (
        <RecurringInvoiceForm
          ...
          onCreated={created => { setItems(prev => [created, ...prev]); setShowForm(false) }}
        />
      )}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-20">
            ...
            <p className="text-xs text-slate-400 mt-1 text-center max-w-xs">
              Vytvořte šablonu — Fakturo podle ní automaticky vystaví a odešle fakturu.
            </p>
          </div>
        ) : (
              ...
              <div key={r.id} className="flex items-center gap-3 px-4 md:px-6 py-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5 truncate">
                    {r.client_name} · {recurrenceLabel[r.recurrence]} · příští {formatDate(r.next_date)}
                  </p>
                </div>
